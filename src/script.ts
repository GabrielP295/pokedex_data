...
type PokeStat = { base_stat: number, stat: { name: string } };

async function fetchResponse(nameOrId: string | number, apiUrl: string): Promise<any> {
...
async function getPokemon(nameOrId: string | number): Promise<Pokemon> {
    ...
    const pokemon: Pokemon = {
        ...
        pokemonTypes: data["types"].map((type: any) => type["type"]["name"]), //.map goes through each index in the array and retrieves needed info
...
async function getPokemonDesc(nameOrId: number | string): Promise<string> {
...
function getPokemonStat(data: PokeStat[], statName: string): number {
    const found = data.find(s => s?.stat?.name === statName);
    return found?.base_stat ?? 0;
}

function renderPokemon(p: Pokemon, container: HTMLDivElement) {
  const primaryType = p.pokemonTypes[0] as keyof typeof TYPE_COLORS;
    ...
    <div class="pokemon-container" style="background-color: ${bgColor}">
      ...
      <div class="type-bar">
        ${p.pokemonTypes
          .map(
            (type) =>
              `<span class="type-badge" style="background-color: ${TYPE_COLORS[type]}">
...
function firstLetterInWordToUppercase(word: string): string {
...
const input = document.querySelector(".pokemon-search-bar") as HTMLInputElement;
const display = document.querySelector(".pokemon-display") as HTMLDivElement;
